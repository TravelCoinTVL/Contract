...
// Copyright (c) 2022 Travel Coin. All rights reserved.
// Travel Coin licenses this file to you under the MIT license.
...
contract TravelCoin is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    TravelCoinDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //BUSD

    uint256 public _totalSupply = 7_777_777_777_777_770 * (10**9);
    ...
    constructor() public BEP20("Travel Coin", "TVL") {

        dividendTracker = new TravelCoinDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TravelCoin: The dividend tracker already has that address");

        TravelCoinDividendTracker newDividendTracker = TravelCoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TravelCoin: The new dividend tracker must be owned by the TravelCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TravelCoin: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TravelCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TravelCoin: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TravelCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TravelCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TravelCoin: Cannot update gasForProcessing to same value");
...
contract TravelCoinDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TravelCoin_Dividen_Tracker", "TravelCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "TravelCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "TravelCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TravelCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TravelCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TravelCoin_Dividend_Tracker: Cannot update claimWait to same value");
